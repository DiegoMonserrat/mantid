...
Assuming that the following files exist:

.. testcode:: Ex
	
	001425.nxs - sample
	001431.nxs - sample transmission
	001427.nxs - direct beam and transmission
	001420.nxs - B4C
	001422.nxs - empty cell
	001428.nxs - empty cell transmission

The full D33 reduction can be setup as following:

.. testcode:: Ex

	data_dir = "/tmp"
	sample_file = data_dir + "/001425.nxs"
	...
	SetupILLD33Reduction(
		...
		BeamCenterFile="%s/%s" % (data_dir, '001427.nxs'),
		Normalisation="Timer",
		DarkCurrentFile= "%s/%s" % (data_dir, '001420.nxs'),
		TransmissionMethod="DirectBeam",
		TransmissionSampleDataFile= "%s/%s" % (data_dir, '001431.nxs'),
		TransmissionEmptyDataFile= "%s/%s" % (data_dir, '001427.nxs'),
		BckTransmissionEmptyDataFile= "%s/%s" % (data_dir, '001427.nxs'),
		TransmissionBeamRadius = 3,
		TransmissionUseSampleDC=False,
		BackgroundFiles="%s/%s" % (data_dir, '001422.nxs'),
		BckTransmissionSampleDataFile="%s/%s" % (data_dir, '001428.nxs'),
	...
	# Reduce the data
	output=SANSReduction(Filename=sample_file, ReductionProperties="d33",OutputWorkspace="d33out")
